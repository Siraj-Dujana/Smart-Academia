...
// Initialize Resend with API key from Railway environment
const resend = new Resend(process.env.RESEND_API_KEY);

const sendEmail = async ({ to, subject, html }) => {
  try {
    const { data, error } = await resend.emails.send({
      from: 'SmartAcademia <onboarding@resend.dev>', // Resend's free test domain
      to: [to],
      subject: subject,
      html: html,
    });
...
// OTP email template (keep your existing template - no changes needed)
const otpEmailTemplate = ({ fullName, otp, purpose }) => {
  const purposeText =
    purpose === "registration"
      ? "verify your email address and complete your registration"
