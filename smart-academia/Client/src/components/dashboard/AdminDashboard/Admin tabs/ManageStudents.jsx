  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Manage Students</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage student enrollments and academic information</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { icon: "groups", label: "Total Students", value: students.length, color: "bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600" },
          { icon: "corporate_fare", label: "Departments", value: [...new Set(students.map(s => s.department).filter(Boolean))].length, color: "bg-blue-100 dark:bg-blue-900/30 text-blue-600" },
          { icon: "calendar_today", label: "Semesters", value: [...new Set(students.map(s => s.semester).filter(Boolean))].length, color: "bg-purple-100 dark:bg-purple-900/30 text-purple-600" },
        ].map(stat => (
          <div key={stat.label} className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
                <tr>
                  {["CMS ID & Student","Department","Semester","Joined","Actions"].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={student._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="size-9 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-700 dark:text-indigo-300 font-bold text-sm flex-shrink-0">
                    ...
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{student.department || "N/A"}</td>
                    <td className="px-4 py-3">
