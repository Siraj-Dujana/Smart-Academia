  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Manage Teachers</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage teacher accounts and information</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative">
            ...
            <input type="text" placeholder="Search teachers by name, email, or CMS ID..."
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
                <tr>
                  {["CMS ID & Teacher","Department","Specialization","Qualification","Joined","Actions"].map(h => (
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map(teacher => (
                  <tr key={teacher._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 font-mono">ID: {teacher.employeeId || "N/A"}</p>
                          <p className="text-xs text-gray-500">{teacher.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{teacher.department || "N/A"}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400 text-xs">{teacher.qualification || "N/A"}</td>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-500">No teachers found</td></tr>
                )}
              </tbody>
            </table>
          </div>
