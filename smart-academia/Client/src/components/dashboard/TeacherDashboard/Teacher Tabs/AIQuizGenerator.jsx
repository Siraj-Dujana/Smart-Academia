...
const AIQuizGenerator = ({ quiz, onQuestionsGenerated, onClose }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // ✅ FIXED: calls correct AI generate endpoint
      ...
      // ✅ FIXED: server returns saved questions directly — show them for preview
            ...
            <div>
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                <h3 className="font-semibold text-gray-900 dark:text-white text-sm sm:text-base">
                  ✅ {generatedQuestions.length} Questions Added to Quiz
                </h3>
                <span className="text-[10px] sm:text-xs text-green-600 font-medium bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded-full">
                  Saved successfully
                </span>
              </div>
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                {generatedQuestions.map((q, i) => (
                  <div key={q._id || i} className="p-3 sm:p-4 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50">
                    <div className="flex items-start gap-2 mb-2 sm:mb-3">
                      ...
                      <p className="text-xs sm:text-sm font-medium text-gray-900 dark:text-white">{q.questionText}</p>
                    </div>
                    <div className="space-y-1 ml-5 sm:ml-8">
                      {q.options?.map((opt, j) => (
                        <div
                          key={j}
                          className={`flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs px-2 py-1 rounded-lg ${
                            opt === q.correctAnswer
                              ? "bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300"
                              : "text-gray-500 dark:text-gray-400"
                          }`}
                        >
                          <span className="flex-shrink-0">{opt === q.correctAnswer ? "✓" : `${String.fromCharCode(65 + j)}.`}</span>
              ...
              <button
                onClick={onClose}
                className="w-full mt-3 sm:mt-4 py-2.5 sm:py-3 px-4 rounded-xl text-sm font-medium text-white bg-green-600 hover:bg-green-700 transition-colors"
              >
                Done — Close
              </button>
            </div>
